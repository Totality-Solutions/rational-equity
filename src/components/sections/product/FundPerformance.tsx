        ...
        <div className="border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="divide-y divide-gray-50 ">
            {data.map((row, index) => (
              ...
              >
                <span className={`text-[15px] ${row.period === "Since Inception" ? "font-bold text-gray-900" : "text-gray-500"}`}>
                  {row.period}
                </span>
                
                <span className={`text-[16px] font-bold text-center ${row.fundReturn.includes('+') ? 'text-green-600' : 'text-gray-900'}`}>
                  {row.fundReturn}
                </span>
