...
export default function Testimonials() {
  ...
  return (
    <section className="bg-white pb-16 md:pb-16 overflow-hidden font-sans">
          ...
          {[...TESTIMONIALS, ...TESTIMONIALS, ...TESTIMONIALS].map((item, index) => (
            ...
            >
              <div className="relative bg-white rounded-2xl shadow-xl shadow-gray-100 flex flex-col h-full overflow-hidden border border-gray-100 transition-all duration-500 hover:shadow-2xl hover:border-brand-maroon/20">
                ...
                {/* Footer Bar */}
                <div className="bg-[#8B0000] p-4 flex items-center gap-4">
                  <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-white flex-shrink-0 flex items-center justify-center text-[#8B0000] font-bold text-base md:text-lg">
                    {item.initial}
                  </div>
                  <div className="overflow-hidden">
                    ...
                    <p className="text-rose-100/70 text-[10px] md:text-xs truncate  tracking-widest font-medium">
                      {item.location}
                    </p>
                  </div>
                </div>
