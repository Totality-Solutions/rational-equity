      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          ...
          <ul className="flex items-center gap-6 xl:gap-10">
            {NAV_LINKS.map(({ label, href, subMenu }) => (
              <li key={label} className="relative group py-5">
                {subMenu ? (
                  <div className="text-body-lg font-sans font-weight-medium text-black transition-colors duration-200 cursor-default hover:text-brand-maroon flex items-center gap-1">
                    {label}
                  </div>
                ) : (
                  <Link
                    href={href}
                    className={`text-body-lg font-sans font-weight-medium transition-colors duration-200 whitespace-nowrap hover:text-brand-maroon ${pathname === href ? 'text-brand-maroon' : 'text-black'
                      }`}
                  >
                    {label}
                  </Link>
        ...
        <div className="flex lg:hidden items-center gap-4">
          <Link href="tel:+9112345678" className="text-brand-maroon p-1">
            <span className="text-sm font-weight-bold border border-brand-maroon px-2 py-1">CALL</span>
          </Link>
          <button
      ...
      >
        <ul className="px-6 py-8 space-y-6 font-sans">
              ...
              {subMenu ? (
                <div className="flex flex-col">
                  ...
                  <button
                    ...
                    className="flex items-center justify-between w-full text-xl font-weight-bold text-gray-800 focus:outline-none transition-colors duration-300"
                  >
                    <span>{label}</span>
                    <svg
                    ...
                    <div className="bg-white border border-gray-100 shadow-sm flex flex-col divide-y divide-brand-maroon/10">
                      {subMenu.map((sub) => (
                        <Link
                          ...
                          className="block px-4 py-5 text-body-md text-gray-800 font-sans font-weight-medium text-center transition-all duration-300 active:bg-[#ffe4e6] active:text-brand-maroon active:font-weight-bold"
                        >
                          {sub.label}
                        </Link>
                      ))}
                ...
                <Link
                  href={href}
                  onClick={() => setIsOpen(false)}
                  className={`text-xl font-weight-bold block transition-colors ${isActive(href) ? 'text-brand-maroon' : 'text-gray-800'
                    }`}
                >
                  {label}
                </Link>
          ...
          <li className="pt-4 flex items-center gap-6">
            <Link href="https://wa.me/yournumber" className="flex items-center gap-2 text-[#00a884] font-weight-bold text-lg">
