...
export default function Navbar() {
  ...
  return (
    <header
      className={`sticky top-0 left-0 right-0 z-50 transition-all duration-300 bg-white/95 backdrop-blur-sm border-b border-b-gray-200 ${scrolled ? ' py-3 md:py-4' : 'py-3 md:py-4'
        }`}
    >
            ...
            {NAV_LINKS.map(({ label, href, subMenu }) => (
              <li key={label} className="relative group py-5">
                {subMenu ? (
                  ...
                  <Link
                    href={href}
                    className={`text-[18px] font-sans font-medium transition-colors duration-200 whitespace-nowrap hover:text-brand-maroon ${pathname === href ? 'text-brand-maroon' : 'text-black'
                      }`}
                  >
                    {label}
                  </Link>
