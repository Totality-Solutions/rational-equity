...
const NAV_LINKS = [
  { label: 'About Us', href: '/about' },
  {
    ...
    href: '#', 
    subMenu: [
      { label: 'India Long-Only Fund', href: '/products/india' },
      { label: 'Gold & Silver Miners Fund', href: '/products/gold' },
      { label: 'Absolute Return Funds', href: '/products/absolute' },
    ]
  },
...
export default function Navbar() {
  ...
  // State to track if the Products sub-menu is open on mobile
          ...
          <ul className="flex items-center gap-6 xl:gap-10">
            {NAV_LINKS.map(({ label, href, subMenu }) => (
              <li key={label} className="relative group py-5">
                {subMenu ? (
                  <div className="text-[18px] font-sans font-medium text-black transition-colors duration-200 cursor-default hover:text-brand-maroon flex items-center gap-1">
                    {label}
                  </div>
                ) : (
                  <Link
                    ...
                    className={`text-[18px] font-sans font-medium transition-colors duration-200 whitespace-nowrap hover:text-brand-maroon ${
                      pathname === href ? 'text-brand-maroon' : 'text-black'
                    }`}
                  >
                    {label}
                  </Link>
                    ...
                    <ul className="bg-white border border-gray-100 shadow-2xl overflow-hidden">
                      {subMenu.map((sub, index) => (
                        <li
                          key={sub.href}
                          className={`${
                            index !== subMenu.length - 1 ? 'border-b border-brand-maroon/20' : ''
                          }`}
                        >
                          <Link
                            href={sub.href}
                            className="block px-4 py-6 text-[16px] text-gray-800 font-sans font-medium text-center transition-all duration-300 hover:bg-[#ffe4e6] hover:text-brand-maroon hover:font-bold"
                          >
                            {sub.label}
                          </Link>
                        </li>
            ...
            <li key={label}>
              {subMenu ? (
                <div className="flex flex-col">
                  {/* Clickable Header for Products */}
                  <button 
                    onClick={() => setIsMobileSubMenuOpen(!isMobileSubMenuOpen)}
                    className="flex items-center justify-between w-full text-xl font-bold text-gray-800 focus:outline-none"
                  >
                    <span>{label}</span>
                    <svg 
                      className={`w-4 h-4 transition-transform duration-300 ${isMobileSubMenuOpen ? 'rotate-180' : ''}`} 
                  ...
                  {/* Sub-menu Items - only visible if toggled */}
                  <div className={`mt-4 pl-4 space-y-4 border-l-2 border-brand-maroon/20 overflow-hidden transition-all duration-300 ${
                    ...
                    {subMenu.map((sub) => (
                      <Link
                        ...
                        className="block text-lg font-medium text-gray-600 hover:text-brand-maroon"
                      >
                        {sub.label}
                      </Link>
                    ))}
