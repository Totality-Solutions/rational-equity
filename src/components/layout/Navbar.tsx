...
import type { SanityHeader } from '@/sanity/queries';

// Product sub-menu icons are fixed brand assets, not editorial content — kept in code
// and matched to the submenu item by its href.
const SUBMENU_ICONS: Record<string, string> = {
  '/product/india-long-only': '/images/icons/india-long-only-fund.svg',
  '/product/gold-silver-miners': '/images/icons/gold-&-silver-miners-fund.svg',
  '/product/absolute-return': '/images/icons/absolute-return-fund.svg',
};
...
const defaultNavLinks = [
  ...
  { label: 'Investment Approach', href: '/investment-approach', submenu: null },
...
export default function Navbar({ header }: { header?: SanityHeader }) {
  ...
  const whatsappLink = header?.whatsappLink || defaultWhatsappLink;
  const ctaText = header?.ctaText || 'Call us';
  const ctaLink = header?.ctaLink || '/contact';
                      ...
                      <div className="absolute left-1/2 -translate-x-1/2 top-full w-[320px] opacity-0 invisible translate-y-2 group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
                        <ul className="bg-white border border-gray-100 rounded-xl shadow-2xl overflow-hidden p-4">
                          {link.submenu.map((sub,ind) => (
                            ...
                            >
                              {SUBMENU_ICONS[sub.href] && (
                                <Image src={SUBMENU_ICONS[sub.href]} alt="Arrow right" width={100} height={100} className="w-7 h-7" />
                              )}
                              <div
      ...
      >
        <ul className="px-6 py-8 space-y-6 font-sans">
          ...
          <li className="pt-4 flex items-center gap-6">
            <Link href={whatsappLink} className="flex items-center gap-2 text-[#00a884] font-bold text-lg">
